...
namespace Microsoft.EntityFrameworkCore.Mongo.Adapter.Serialization
{
    /// <inheritdoc />
    /// <summary>
    ...
    public class DenormalizingBsonClassMapSerializer<TClass> : BsonClassMapSerializer<TClass>
    {
        ...
        public DenormalizingBsonClassMapSerializer(
            BsonClassMap<TClass> bsonClassMap,
            IEnumerable<string>? denormalizedMemberNames = null)
            : base(bsonClassMap)
        {
            _classMap = bsonClassMap;
            DenormalizedMemberMaps = (denormalizedMemberNames ?? Array.Empty<string>())
        ...
        private void SerializeClass(BsonSerializationContext context, BsonSerializationArgs args, TClass document)
        {
            ...
            foreach (var memberMap in DenormalizedMemberMaps)
            {
        ...
        {
            var discriminatorConvention = BsonSerializer.LookupDiscriminatorConvention(_classMap.ClassType);
            var discriminator = discriminatorConvention?.GetDiscriminator(nominalType, obj.GetType());
